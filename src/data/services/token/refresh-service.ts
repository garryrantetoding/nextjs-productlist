...
//   const TokenRefresh = await getRefreshTokenFromCookies();  // Retrieve the refresh token from cookies

//   console.log("Sending refresh token", TokenRefresh);  // Optional: Logging the refresh token

//   try {
//     // Use axiosInstance to make the POST request for refreshing the access token
//     const response = await axios.post(url, {
//       refreshToken: TokenRefresh,  // Send the refresh token to backend
//     }, {
...
//       console.log("New access token received:", response.data.accessToken);  // Optional: Log the new access token
...
export async function refreshAccessToken() {
  ...
  console.log("Sending refresh token", TokenRefresh);  // Debugging refresh token
  ...
  try {
    ...
    const response = await axios.post(url, { refreshToken: TokenRefresh }, {
    ...
    if (response.data.accessToken) {
      console.log("New access token received:", response.data.accessToken);  // Debugging new token
...
export async function LogoutBackend() {
  ...
  console.log("Sending refresh token for logout", TokenRefresh);  // Optional: Logging the refresh token

  try {
    // Send the refresh token to the backend for logout
    const response = await axiosInstance.post(url, {
      refreshToken: TokenRefresh,  // Send the refresh token for logout
    }, {
...
//     console.log("qesdiss", TokenRefresh);
