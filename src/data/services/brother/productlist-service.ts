...
import { Role, Status, Deleted } from "../enum/enum";
import { getAccessTokenFromCookies } from "../token/get-token";
...
// Create a function to send product data to the backend
export async function  AddProductList(product: ProductListProps) {
    const url = "/product";
    ...
    formData.append('image', product.image);  // 'image' corresponds to the backend field
  
    // Append other product fields to the FormData
    // formData.append('id', product.id.toString());
    formData.append('brochure', product.brochure.toString());
    formData.append('productName', product.productName);
    formData.append('category', product.category);
    formData.append('customerPrice', product.customerPrice);
    formData.append('resellerPrice', product.resellerPrice);
    formData.append('description', product.description);
    formData.append('videoTutorial', product.videoTutorial);
...
export async function LoadProductList(page: number, pageSize: number,productNameSearch: string, categoryFilter: string) {
    const url = "/product";
    // const accessToken = await getAccessTokenFromCookies();
    // console.error('edcr', accessToken);
        const accessToken = await ensureValidAccessToken();
    ...
    try {
        ...
        const response = await axiosInstance.get(url, {
            params:{ page, pageSize,productNameSearch:productNameSearch,categoryFilter:categoryFilter},
            headers: {
                Authorization: `Bearer ${accessToken}`, // Using the retrieved token
            },
            withCredentials: true,  // Ensure cookies are sent with the request (if you're using cookies)
...
export async function EditProductList(id: number, brochure: boolean,
    productName: string,
    category: string,
    customerPrice: string,
    resellerPrice: string,
    ...
    const url = `/product/${id}/data`; // The endpoint you're calling
...
export async function EditProductImage(id: number, image: File,

) {
    const url = `/product/${id}/image`; // The endpoint you're calling
    const accessToken = await ensureValidAccessToken(); // Use ensureValidAccessToken
    ...
    // Append the image to the FormData
    formData.append('image', image);  // 'image' corresponds to the backend field
