...
function ReportOrderList({ invoiceNumberSearch, rolesFilter, taskStatusFilter, orderStatusFilter, startDate, endDate, statusFilter, data, setData, selectedRows, setSelectedRows, page, pageSize }: ReportOrderListProps) {
  ...
  const openEditResellerModal = async (reportorder: EditReportOrderList) => {
    ...
    console.log("testedit",convertedProduct)
  ...
  const handleSaveImageConfirm = async () => {
    if (editReportOrder && editReportOrder.invoice) {
      try {
    ...
    if (editReportOrder) {
      try {
        ...
        console.log("Submitting edited data:", {
          id: editReportOrder.id,
          orderStatus: editReportOrder.orderStatus,
          paymentStatus: editReportOrder.paymentStatus,
          taskStatus: editReportOrder.taskStatus,
          notes: editReportOrder.notes,
        });
        ...
        const updatedProductList = await EditReportOrderList(
          ...
          editReportOrder.orderStatus,
          editReportOrder.paymentStatus,
          editReportOrder.taskStatus,
          editReportOrder.notes,
  ...
  const columns = React.useMemo(() => [
    ...
    columnHelper.accessor('invoice', {
      cell: info => {
        const invoice = info.getValue(); // Get the image value from the column
        return (
      
          <a
            href={`${process.env.NEXT_PUBLIC_BACKEND_URL}/${invoice}`} 
            target="_blank"
    ...
    columnHelper.accessor('orderStatus', {
      cell: info => {
        const orderStatus = info.getValue();  
