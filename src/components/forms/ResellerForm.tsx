...
import ResellerList from '../custom/Reseller/ListReseller';
import StatusFilter from '../custom/Reseller/filters/statusfilter'; // Import StatusFilter
import SearchBar from '../custom/Reseller/filters/searchbar';
import AddUserButton from '../custom/Reseller/buttons/add-user-button';
import DeleteUserButton from '../custom/Reseller/buttons/delete-user-button';
import ProductListModal from '../custom/Reseller/popups/popup-add/popup-addReseller';
import DiscardConfirmationModal from '../custom/Reseller/popups/popup-add/popup-discardadd'; // Import the DiscardConfirmationModal
import DeleteConfirmationModal from '../custom/Reseller/popups/popup-deleteuser';
...
import RoleFilter from '../custom/Reseller/filters/rolefilter';
import { AddResellerList, LoadResellerList } from '@/data/services/brother/reseller-service';
import ResellerListModal from '../custom/Reseller/popups/popup-add/popup-addReseller';
...
interface BackendErrorReseller {
...
const ResellerDashboard: React.FC = () => {
  ...
  const [data, setData] = useState<ResellerListprop[]>([]); // Initially empty, will be populated by backend data
  // const [permissionauth, setPermissionauth] = useState<any[]>([]); // Initially empty, will be populated by backend data
  const [backendError, setBackendError] = useState<BackendErrorReseller>({
    storePhoto: '',
      shopName: '',
      owner: '',
      roles: '',
  ...
  const [newProductList, setNewProductList] = useState<ListReseller>({
    storePhoto: new File([], ""),
      shopName: '',
      owner: '',
      roles: '',
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        console.log("filtertest",page, pageSize,shopNameSearch, roleFilter)
        const fetchedData = await LoadResellerList(page, pageSize,shopNameSearch, roleFilter);
...
//   deleted: Deleted.Yes,}]);
    ...
    fetchData();
  }, [page, pageSize, shopNameSearch, roleFilter, statusFilter]); // Empty dependency array to run this effect only once when the component mounts.
  ...
  const handleAddResellerList = async (
    storePhoto: File,//image
  shopName: string,//productname
  owner: string,//category
  roles: string,//customerprice
  ...
  printerA4: boolean,//brochure
    ...
    const newProductList: ListReseller = {
      storePhoto,
      shopName,
        owner,
        roles,
    ...
    try {
      ...
      const response = await AddResellerList(newProductList);
...
>
      {/* {Object.keys(selectedRows).length === 0 ? (
        <> */}
          <div className="flex justify-between items-center mb-4 w-full">
            ...
            {/* Add User Button */}
            <div className="flex space-x-4 ">
            <SearchBar searchQuery={shopNameSearch} setSearchQuery={setShopnameSearch} />
      ...
      <ResellerListModal 
        ...
        onAddProductList={handleAddResellerList} 
