...
const NotifButton: React.FC = () => {
  ...
  const [isClient, setIsClient] = useState(false); // flag to confirm client-side
  ...
  useEffect(() => {
    ...
    const storedState = localStorage.getItem('notifButtonState');
    if (storedState) {
      setIsClicked(JSON.parse(storedState));
    }
  }, []);
  ...
  return (
    <button onClick={handleClick} className="ml-4 p-4 rounded-full"
    style={{color: "rgba(31, 65, 115, 0.5)"
    }}>
      {isClicked ? (
